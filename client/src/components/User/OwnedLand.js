...
function OwnedLand() {
  let [account,setAccount] = useState(null);
    ...
    const display=async()=>{
      let data = await contract.methods.myVerifiedLands().call({ from: account[0] });
      setList(data);
      console.log(data);
      
    }
    const viewDetails=async(land)=>{
      let data = await contract.methods.viewLandDetails(land).call();
      console.log(data);
